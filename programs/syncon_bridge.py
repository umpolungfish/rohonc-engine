"""
SynthOmnicon bridge — cross-system structural distances in IG notation.

Computes pairwise distances between manuscript crystal addresses using the
...
SYSTEMS: dict[str, list[int]] = {
    ...
    'OS synthon':       [1, 3, 2, 4, 2, 1, 2, 2, 1, 2, 2, 2],
}
...
CRYSTAL_NOTATION: dict[str, str] = {
    ...
    'OS synthon': '⟨ Ð_C  Þ_¨  Ř_Ť  Φ_}  ƒ^ż  Ç^W  Γ_ʔ  ɢ^ˌ  ⊙_ÿ  Ħ_A  Σ_ï  Ω_z ⟩',
}
...
def format_tuple(name: str, t: list[int]) -> str:
    vals = '  '.join(IG_VALUE_NAMES[i][v] for i, v in enumerate(t))
    return f'{name:<14}  ⟨ {vals} ⟩  tier={tier(t)}'
...
def main() -> None:
    ...
    print('=== CRYSTAL ADDRESSES (IG notation) ===\n')
    for name, t in SYSTEMS.items():
        print(f'  {name}')
        print(f'    {CRYSTAL_NOTATION[name]}')
        print(f'    tier = {tier(t)}\n')

    print('=== PAIRWISE SYNCON DISTANCES ===\n')
    print(f'  {"":14}  ' + ''.join(f'{n:>14}' for n in names))
    for a in names:
        row = f'  {a:<14}  '
        for b in names:
            d = distance(SYSTEMS[a], SYSTEMS[b])
            row += f'{d:>14.4f}'
        print(row)
    ...
    for a, b in pairs:
        cs = conflict_set(SYSTEMS[a], SYSTEMS[b])
        d  = distance(SYSTEMS[a], SYSTEMS[b])
        print(f'  {a} ↔ {b}')
        print(f'    d = {d:.4f}   conflicts: {{{", ".join(cs) if cs else "∅"}}}')

    print('\n=== SIX-SYSTEM MEET (OS synthon + Linear A) ===\n')
    meet = [min(SYSTEMS['OS synthon'][i], SYSTEMS['Linear A'][i]) for i in range(12)]
    vals = '  '.join(IG_VALUE_NAMES[i][v] for i, v in enumerate(meet))
    print(f'  MEET(OS_synthon, Linear_A) = ⟨ {vals} ⟩')
    print(f'  tier = {tier(meet)}')
    if meet == SYSTEMS['OS synthon']:
